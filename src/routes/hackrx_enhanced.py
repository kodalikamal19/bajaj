...
hackrx_enhanced_bp = Blueprint("hackrx_enhanced", __name__)
...
@hackrx_enhanced_bp.route('/v1/hackrx/run', methods=['POST'])
@cross_origin()
def hackrx_enhanced_run():
    """Enhanced HackRX API endpoint with improved accuracy"""
    ...
    try:
        ...
        print(f"Unexpected error in hackrx_enhanced_run: {str(e)}")
        memory_manager.force_garbage_collection()
        return jsonify({'error': 'Internal server error'}), 500

@hackrx_enhanced_bp.route('/health', methods=['GET'])
@cross_origin()
